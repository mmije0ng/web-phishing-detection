# (확장용) 분석 결과 응답 dto
def simple_response_dto(url, prediction_result, prediction_prob):
    """Response Body에 맞는 DTO 생성."""

    return {
        "url": url,
        "prediction_result": int(prediction_result),  # 피싱 여부
        "prediction_prob": f"{prediction_prob}%",  # 피싱 확률
    }

# (웹용) 상세 분석 결과 응답 dto
def detailed_response_dto(url, prediction_result, prediction_prob, suspicious_features, ip_info):
    """Response Body에 맞는 DTO 생성."""
