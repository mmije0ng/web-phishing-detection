...
def not_blacklist_detailed_analyze_url(db, url):
    ...
    return detailed_analyze_url(db, url, url_id, url_exist)
...
# (웹용) 상세 분석 결과
async def detailed_analyze_url(db, url, url_id, url_exist):
    """URL에 대해 피처 추출 및 분석을 실행하는 함수."""
    ...
    # DTO 생성
    detailed_response_dto = url_response_dto.detailed_response_dto(url, prediction_result, prediction_prob, suspicious_features, ip, country, port_list)
